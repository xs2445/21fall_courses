...
def value_iteration(env, gamma, max_iteration, theta):
    ...
    while delta>=theta and numIterations < max_iteration:
    # while delta >= theta:
        delta = 0

        V_previous = V.copy()
        # Bellman Expectation
        q_sa = action_evaluation(env, gamma, V)
        # Bellman Optimality
        V = np.max(q_sa, axis=1)

        delta = max(delta, np.max(abs(V-V_previous)))
...
def extract_policy(env, v, gamma):
    ...
    policy = np.argmax(action_evaluation(env, gamma, v), axis=1)
