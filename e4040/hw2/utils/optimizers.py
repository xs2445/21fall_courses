...
class AdaGradOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            params[k] -= (learning_rate/(eps + np.sqrt(cache[k])))*grads[k]
...
class AdamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            # bias correction
            unbiased_mom = momentums[k] / (1-beta1**t)
            unbiased_vel = velocitys[k] / (1-beta2**t)
            # update the weight
            params[k] -= learning_rate*unbiased_mom / (np.sqrt(unbiased_vel)+eps)
...
class NadamOptim(Optimizer):
    ...
    def step(self, model, learning_rate):
        ...
        for k in grads:
            ...
            unbiased_vel = velocitys[k] / (1-beta2**t)
            # update the weight
            params[k] -= learning_rate*(beta1*unbiased_mom + (1-beta1)*grads[k]/(1-beta1**t))/(np.sqrt(unbiased_vel)+eps)
