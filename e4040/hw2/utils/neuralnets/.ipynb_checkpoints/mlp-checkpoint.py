...
class MLP(object):
    ...
    def __init__(self, input_dim=3072, hidden_dims=[200, 200], num_classes=10, weight_scale=1e-2,
        ...
        for i in range(len(hidden_dims)):
            ...
            # randomly assign the weight parameter
            W = weight_scale * np.random.rand(dims[i], dims[i + 1])
            # randomly assign the bia parameter
            b = np.zeros(dims[i + 1])
            # append weight and bias to the model
    ...
    def loss(self, X, y):
        ...
        for i in range(num_layers - 1):
            ...
            if use_bn:
                # save dictionary bn_config of each layers in the bn_params as list
                bn_params[i]["bn_config_{}".format(i)] = dict()
            ...
            if dropout_config['enabled'] == True:
